...
uint8_t input_int(char *p_msg, uint16_t *p_int_nr)
{
	...
	uint8_t key;						//Man måste ge plats till '\0' i med att det är en string som vi har att göra */
	...
	while (1) {
		...
		if (key == '#') {					// confirmation of the number?
			...
			if (length > 0) {
				...
				length--;
				lcd_write(INSTRUCTION,0x10); //Cursor flyttas ett steg till vänster
				lcd_write(DATA,' ');		// Sudda bort 
		...
		while(numkey_read() != NO_KEY); //UPPGIFT: Skriv kod så att koden stannar/"loopar" tills att numkey_read() inte längre detekterar en knapptryckning!
